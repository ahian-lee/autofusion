...
WORKERS="${WORKERS:-8}"
SEED="${SEED:-0}"
SAVE_ROOT="${SAVE_ROOT:-./outputs/constructor_occ_baseline}"
...
${PYTHON_BIN} ./tools/constructor_occ_baseline.py \
  ...
  --workers "${WORKERS}" \
  --seed "${SEED}" \
  --save_dir "${SAVE_ROOT}/topo_sdf_occ"
